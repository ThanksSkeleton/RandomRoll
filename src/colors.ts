...
export type PaletteRole = "costumePrimary" | "costumeSecondary" | "costumeAccent" | "hair" | "eyes" | "power";
export type RecipeColorKind = "input" | "inputDesat" | "inputDesatDark" | "analog1" | "analog2" | "complement" | "rightAngle1" | "rightAngle2" | "splitComplement1" | "splitComplement2" | "white" | "black" | "primary" | "N/A";
...
const SCHOOL_UNIFORM_COLORS: Array<{ roleKey: PaletteRole; layer: DollRole; label: string; role: string; hexKey: keyof Pick<PaletteConstants, "schoolUniformPrimaryHex" | "schoolUniformSecondaryHex" | "schoolUniformHighlightHex"> }> = [
...
export const fmt = (value: number, digits = 2): string => Number(value).toFixed(digits).replace(/\.?0+$/, "");

// Converts an 8-bit sRGB channel into linear light for color-space math.
function srgbToLinear(v: number): number { v = v / 255; return v <= 0.04045 ? v / 12.92 : Math.pow((v + 0.055) / 1.055, 2.4); }
// Converts a linear-light channel back into an 8-bit sRGB channel.
function linearToSrgb(v: number): number { v = clamp(v, 0, 1); return Math.round(255 * (v <= 0.0031308 ? 12.92 * v : 1.055 * Math.pow(v, 1 / 2.4) - 0.055)); }
// Parses a six-digit hex color string into separate sRGB channels.
function hexToRgb(hex: string): { r: number; g: number; b: number } { const clean = hex.replace("#", ""); return { r: parseInt(clean.slice(0, 2), 16), g: parseInt(clean.slice(2, 4), 16), b: parseInt(clean.slice(4, 6), 16) }; }
...
export function oklchLabel(c: Pick<PaletteColor, "h" | "l" | "c">): string { return c.h === null ? `OKLCH H none L ${fmt(c.l)} C ${fmt(c.c, 4)}` : `OKLCH H ${fmt(c.h)} L ${fmt(c.l)} C ${fmt(c.c, 4)}`; }

// Creates a normalized palette color object and computes its display hex value.
function makeColor(label: string, role: string, group: string, variant: string, h: number | null, l: number, c: number): PaletteColor {
...
function makeFixedColor(label: string, role: string, group: string, variant: string, hex: string): PaletteColor {
...
export function buildPalettes(k: PaletteConstants, recipes: PaletteRecipe[], options: PaletteBuildOptions = {}): BuiltPalette[] {
  ...
  function importantColor(kind: RecipeColorKind, roleKey: PaletteRole): ImportantColor {
    if (kind === "N/A") return { kind, label: "N/A", hex: "transparent", h: null, l: null, c: null };
    if (kind === "primary") kind = "input";
    if (kind === "white") return { kind, label: labels.white, h: null, l: k.lPureWhite, c: 0, hex: oklchToHex(k.lPureWhite, 0, 0) };
    if (kind === "black") return { kind, label: labels.black, h: null, l: k.lPureBlack, c: 0, hex: oklchToHex(k.lPureBlack, 0, 0) };
    if (kind === "inputDesat") return { kind, label: labels.inputDesat, h: H, l: k.lOffWhiteLight, c: k.cOffWhite, hex: oklchToHex(k.lOffWhiteLight, k.cOffWhite, H) };
    if (kind === "inputDesatDark") return { kind, label: labels.inputDesatDark, h: H, l: k.lOffBlackLight, c: k.cOffBlack, hex: oklchToHex(k.lOffBlackLight, k.cOffBlack, H) };
    const h = hues[kind];
  ...
  function colorFromImportant(name: string, role: string, variant: string, important: ImportantColor, l: number, c: number): PaletteColor {
  ...
  function packageFor(roleKey: PaletteRole, kind: RecipeColorKind): PaletteColor[] {
    ...
    if (roleKey === "eyes") return [colorFromImportant(spec.label, spec.role, important.label, important, important.l, important.c)];
    if (roleKey === "power") {
      return [
  ...
  function buildPalette(name: string, note: string, roleMap: Record<PaletteRole, RecipeColorKind>): BuiltPalette {
    const orderedRoles: PaletteRole[] = ["costumePrimary", "costumeSecondary", "costumeAccent", "hair", "eyes", "power"];
    ...
    if (options.schoolUniformOverride) {
      ...
      for (let i = colors.length - 1; i >= 0; i--) {
        if (costumeRolePrefixes.some(role => colors[i].role === role || colors[i].role.startsWith(`${role}_`))) colors.splice(i, 1);
      }
...
export function generate_palette(inputColor: string, recipes: PaletteRecipe[], options: PaletteBuildOptions = {}, constants: PaletteConstants = DEFAULT_PALETTE_CONSTANTS): BuiltPalette[] {
