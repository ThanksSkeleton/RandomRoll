...
const PAPER_ROLES: Array<[DollRole, string]> = [["main","Main Costume"],["support","Supporting Costume"],["accent","Accent / Highlight"],["hair","Hair + Eyebrows"],["eyes","Eyes"],["skin","Skin"],["power","Power / Stars"]];
const DOLL_CATALOG: DollDefinition[] = [{ kind: "male", label: "Male" }, { kind: "female", label: "Female" }];
...
function readConstants(): PaletteConstants {
  return {
    ...
    skinHex: (document.getElementById("skinColor") as HTMLInputElement | null)?.value ?? DEFAULT_PALETTE_CONSTANTS.skinHex,
    schoolUniformPrimaryHex: (document.getElementById("schoolUniformPrimaryColor") as HTMLInputElement | null)?.value ?? DEFAULT_PALETTE_CONSTANTS.schoolUniformPrimaryHex,
    schoolUniformSecondaryHex: (document.getElementById("schoolUniformSecondaryColor") as HTMLInputElement | null)?.value ?? DEFAULT_PALETTE_CONSTANTS.schoolUniformSecondaryHex,
...
function makeLayer(role: DollRole, url: string): HTMLDivElement { const layer = document.createElement("div"); layer.className = "mask-layer"; layer.dataset.role = role; layer.style.backgroundColor = `var(--role-${role})`; applyMask(layer, url); return layer; }
...
function makeFigure(doll: DollDefinition): HTMLDivElement { const stack = cloneDollTemplate(); stack.ariaLabel = doll.label; const assets = MASK_ASSETS[doll.kind]; for (const role of LAYER_ORDER) stack.appendChild(makeLayer(role, assets[role])); return stack; }

// Wraps a painted doll with the small comparison label used only by this app's carousel.
function makeDollComparison(label: string, doll: HTMLDivElement): HTMLDivElement { const wrap = document.createElement("div"); wrap.className = "doll-comparison"; const caption = document.createElement("div"); caption.className = "doll-comparison-label"; caption.textContent = label; wrap.appendChild(doll); wrap.appendChild(caption); return wrap; }

// Shows the active doll in a small carousel so new doll definitions can be added later.
function renderDollCarousel(dollPair: { school: BuiltPalette["doll"]; full: BuiltPalette["doll"] }): void { const root = el("paperRoot"); const doll = DOLL_CATALOG[activeDollIndex]; if (!doll) throw new Error("No paper dolls are available to render."); root.innerHTML = ""; root.appendChild(makeDollComparison("Student Uniform", paintdoll(doll.kind, dollPair.school))); root.appendChild(makeDollComparison("Full Palette", paintdoll(doll.kind, dollPair.full))); el("dollCarouselLabel").textContent = `${doll.label} ${activeDollIndex + 1} of ${DOLL_CATALOG.length}`; }

// Advances the doll carousel, wrapping around at either end.
function moveDollCarousel(delta: number): void { activeDollIndex = (activeDollIndex + delta + DOLL_CATALOG.length) % DOLL_CATALOG.length; renderDollCarousel(currentDollPair); }

// Writes the active doll colors into CSS custom properties used by the mask layers.
function applyDollPalette(node: HTMLElement, doll: BuiltPalette["doll"]): void { for (const [role] of PAPER_ROLES) node.style.setProperty(`--role-${role}`, doll[role] ?? "transparent"); }

// Public helper: create one painted doll element without mounting it into the app carousel.
export function paintdoll(dolltype: FigureKind, palette: BuiltPalette | BuiltPalette["doll"]): HTMLDivElement {
...
function renderSchoolUniformReference(k: PaletteConstants): void { const root = el("schoolUniformReference"); root.innerHTML = SCHOOL_UNIFORM_SWATCHES.map(color => { const hex = k[color.key as keyof Pick<PaletteConstants, "schoolUniformPrimaryHex" | "schoolUniformSecondaryHex" | "schoolUniformHighlightHex">]; return `<div class="paper-role"><span class="chip" style="background:${hex}"></span><span><strong>${color.label}</strong><code>${hex}</code></span></div>`; }).join(""); }
// Builds the copyable text report for the currently selected palette.
function paletteText(palette: BuiltPalette, k: PaletteConstants): string { const lines = []; lines.push(palette.name.toUpperCase()); lines.push(palette.note); lines.push(""); lines.push(`BASE: OKLCH H ${fmt(k.baseH)} L ${fmt(k.baseL)} C ${fmt(k.baseC, 4)} HEX ${oklchToHex(k.baseL, k.baseC, k.baseH)}`); lines.push(`ANALOG 1: H ${fmt(wrapHue(k.baseH - k.anaOffset))}`); lines.push(`ANALOG 2: H ${fmt(wrapHue(k.baseH + k.anaOffset))}`); lines.push(`COMPLEMENT: H ${fmt(wrapHue(k.baseH + k.compOffset))}`); lines.push(`RIGHT ANGLE 1: H ${fmt(wrapHue(k.baseH + k.rightAngleOffset))}`); lines.push(`RIGHT ANGLE 2: H ${fmt(wrapHue(k.baseH - k.rightAngleOffset))}`); lines.push(`SPLIT COMPLEMENT 1: H ${fmt(wrapHue(k.baseH + k.compOffset - k.splitOffset))}`); lines.push(`SPLIT COMPLEMENT 2: H ${fmt(wrapHue(k.baseH + k.compOffset + k.splitOffset))}`); lines.push(""); lines.push("ROLE MAPPING"); const roleNames = { costumePrimary: "Costume Primary", costumeSecondary: "Costume Secondary", costumeAccent: "Costume Accent", hair: "Hair", eyes: "Eyes", power: "Power" }; for (const [key, label] of Object.entries(roleNames)) { const imp = palette.important[key as keyof typeof roleNames]; lines.push(`${label}: ${imp?.label ?? "N/A"}`); } lines.push(""); lines.push("COMPLETE PALETTE"); for (const c of palette.colors) lines.push(`${c.role}: ${oklchLabel(c)} HEX ${c.hex}`); lines.push(""); lines.push("PAPER DOLL PALETTE"); for (const [role, label] of PAPER_ROLES) lines.push(`${role}: ${palette.doll[role] ?? "transparent"} (${label})`); return lines.join("\n"); }
// Recomputes palettes from the controls and refreshes every dependent UI panel.
function render(syncBaseColor = true): void { const k = readConstants(); if (syncBaseColor && !suppressColorSync) el<HTMLInputElement>("baseColor").value = oklchToHex(k.baseL, k.baseC, k.baseH); const palettes = buildPalettes(k, paletteRecipes); const schoolPalettes = buildPalettes(k, paletteRecipes, { schoolUniformOverride: true }); activeIndex = clamp(activeIndex, 0, palettes.length - 1); const active = palettes[activeIndex]; const schoolActive = schoolPalettes[activeIndex]; if (!active || !schoolActive) throw new Error("No palettes are available to render."); currentDollPair = { school: schoolActive.doll, full: active.doll }; renderPaletteButtons(palettes); el("activeTitle").textContent = `${activeIndex + 1}. ${active.name}`; el("activeNote").textContent = active.note; renderSchoolUniformReference(k); renderDollCarousel(currentDollPair); renderPaperRoles(active.doll); renderSwatches(active); el<HTMLTextAreaElement>("activeOutput").value = paletteText(active, k); }
// Converts the color picker value back into OKLCH controls before rendering.
function syncBaseFromColorPicker(): void { suppressColorSync = true; const o = hexToOklch(el<HTMLInputElement>("baseColor").value); el<HTMLInputElement>("baseH").value = String(Math.round(o.H)); el<HTMLInputElement>("baseL").value = String(Math.round(o.L100)); el<HTMLInputElement>("baseC1000").value = String(Math.round(o.C * 1000)); suppressColorSync = false; render(false); }
